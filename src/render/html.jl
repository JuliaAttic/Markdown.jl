...
const _htmlescape_chars = @compat(Dict('<'=>"&lt;",   '>'=>"&gt;",
                                       '"'=>"&quot;", '&'=>"&amp;",
                                       # ' '=>"&nbsp;",
                                       ))
for ch in "'`!@\$\%()=+{}[]"
    _htmlescape_chars[ch] = "&#$(@compat(Int(ch)));"
end
...
function html(io::IO, code::Code)
    withtag(io, :pre) do
        maybe_lang = code.language != "" ? @compat(Dict(:class=>"language-$(code.language)")) : Dict()
...
function htmlinline(io::IO, md::Image)
    tag(io, :img, Pair(:src, md.url), Pair(:alt, md.alt))
end

function htmlinline(io::IO, link::Link)
    withtag(io, :a, Pair(:href, link.url)) do
...
function writemime(io::IO, ::MIME"text/html", md::MD)
    withtag(io, :div, Pair(:class, "markdown")) do
        html(io, md)
    end
end
